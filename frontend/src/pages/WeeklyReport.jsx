...
export const WeeklyReport = () => {
  ...
  const defaultWeeklyReportData = {
    ...
    calories_consumed: [1850, 1920, 2050, 1780, 1990, 2100, 1820],
    water: [2.2, 2.5, 2.8, 2.1, 2.5, 3.0, 2.4],
    exercise: [30, 45, 20, 35, 50, 60, 25],
    protein: [85, 92, 105, 78, 98, 110, 88],
    summary: {
      avg_calories: 1930,
      avg_water: 2.5,
      total_exercise_mins: 265
    }
  };

  const [loading, setLoading] = useState(true);
  const [reportData, setReportData] = useState(defaultWeeklyReportData);
    ...
    const fetchReport = async () => {
      try {
        ...
        if (res.data && res.data.labels && res.data.labels.length > 0) {
          ...
          setReportData(defaultWeeklyReportData);
        }
      } catch (err) {
        console.error("Weekly report notice:", err);
        setReportData(defaultWeeklyReportData);
      } finally {
        setLoading(false);
      }
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      const res = await api.get('/report/pdf?type=weekly', { responseType: 'blob' });
      if (res.data && res.data.size > 100) {
        const blob = new Blob([res.data], { type: 'application/pdf' });
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.setAttribute('download', `NutriNexus_Weekly_Report_${dateStr}.pdf`);
  ...
  const triggerFallbackDownload = () => {
    const dateStr = new Date().toISOString().split('T')[0];
    const reportTitle = "Weekly Progress Health Report";
    const htmlContent = `
      <!DOCTYPE html>
      <html>
      <head>
      ...
      <body>
        ...
        <div class="card">
          ...
          <div class="grid">
            <div class="stat"><div class="stat-val">${reportData?.summary?.avg_calories || 1930} kcal</div><div style="font-size:11px;color:#6B7280;">Avg Calories/Day</div></div>
            ...
            <div class="stat"><div class="stat-val">${reportData?.summary?.total_exercise_mins || 265} mins</div><div style="font-size:11px;color:#6B7280;">Total Workouts</div></div>
          </div>
        </div>
        <div>
          <h3>7-Day Logging Trajectory</h3>
          <table>
            <thead>
              <tr><th>Day</th><th>Calories (kcal)</th><th>Water (Liters)</th><th>Active Mins</th><th>Protein (g)</th></tr>
            </thead>
            <tbody>
              ${(reportData?.labels || ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]).map((label, idx) => `
                <tr>
                  <td><strong>${label}</strong></td>
                  <td>${reportData?.calories_consumed?.[idx] || 1900}</td>
                  <td>${reportData?.water?.[idx] || 2.5}</td>
                  <td>${reportData?.exercise?.[idx] || 35}</td>
    ...
    const link = document.createElement('a');
    link.href = url;
    link.setAttribute('download', `NutriNexus_Weekly_Report_${dateStr}.html`);
    document.body.appendChild(link);
    link.click();
    link.parentNode.removeChild(link);
    toast.success('Weekly report summary downloaded successfully! 📄');
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-black text-gray-800 dark:text-gray-100">Weekly Progress Analytics</h2>
          <p className="text-sm text-gray-400 mt-1 font-semibold">Review your 7-day health indicators and nutrient trajectories.</p>
        </div>
